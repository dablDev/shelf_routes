...
String params(shelf.Request request, String name) {
...
final _removeBody = shelf.createMiddleware(responseHandler: (r) {
  if (r == null) {
    return null;
  }
...
class Router {
  ...
  void add(String verb, String route, shelf.Handler handler) {
  ...
  void mount(String prefix, Router router, {shelf.Middleware middleware}) {
    middleware = middleware ?? ((shelf.Handler handler) => handler);
    ...
    all(prefix + '<path|[^]*>', (shelf.Request request) {
      return middleware(handler)(request.change(path: path));
    });
  }
