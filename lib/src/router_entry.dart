...
class RouterEntry {
  ...
  RouterEntry(this.verb, this.route, this._handler, {shelf.Middleware middleware}) : _middleware = middleware ?? ((shelf.Handler fn) => fn) {
  ...
  Future<shelf.Response> invoke(shelf.Request request, Map<String, String> params) async {
    request = request.change(context: {'shelf_router/params': params});

    return await _middleware(_handler)(request);
  }
}
